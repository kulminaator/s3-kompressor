...
(defn internal-write-zip-from
  "Keeps on taking elements from channel and writes them to the zipfile.
  Tries to create splitted files if split-size is not nil."
  [transport-channel split-size filename-threaded-base]
  (loop [zip-output (ZipOutputStream. (clojure.java.io/output-stream (make-zip-name filename-threaded-base 0)))
    ...
    (if file-to-add
      (do
        ...
        (with-open [instream (open-stream (:input-stream-fn file-to-add))]
          (clojure.java.io/copy instream zip-output :buffer-size 32768))
        (recur (roll-zip-if-needed zip-output filename-threaded-base (+ written (:size file-to-add)) next-roll)
...
(defn dispatch-internal-zip-writers
  ...
  (mapv #(async/thread (internal-write-zip-from transport-channel split-size %)) filenames))
